...
local function get_state()
	...
	return {
		...
		settings = { lives = 5, beacon_hp = 100, mm_auto_assign = true, auto_start = false, auto_start_delay = 8, countdown = 5, match_duration = 600 },
...
local function get_player_state(name)
	...
	return { team = nil, role = nil, lives = 5, phase = "alive", points = 0, eliminated = false }
end
...
function get_system_formspec(player)
	...
	table.insert(fs, string.format("label[0.4,1.3;%s: %s | %s: %s | %s: %s | %s: %s | %s: %d]",
		...
		S("Lives"), minetest.formspec_escape(tostring(pl.lives or 0)),
