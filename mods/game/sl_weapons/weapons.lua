...
function W.register_weapon(def)
	...
	minetest.register_tool(W.modname .. ":" .. def.item, {
		...
		on_use = function(itemstack, user, pointed_thing)
			...
			-- Ammo from the shared pools
			if def.pool then
				local cost = def.ammo_cost or 1
				if not W.take_ammo(name, def.pool, cost) then
					...
					minetest.chat_send_player(name, S("Dry."))
		...
		on_place = function(itemstack, placer, pointed_thing)
			-- Zoom toggle for scoped weapons (Lance, Repeater)
			if def.zoom and placer and placer.is_player and placer:is_player() then
		...
		on_secondary_use = function(itemstack, user, pointed_thing)
			if def.zoom and user and user.is_player and user:is_player() then
...
function W.autoswitch_pistol(user, fired_def)
	...
	for i = 1, inv:get_size("main") do
		...
		if stack:get_name() == "sl_weapons:" .. fired_def.item then
			inv:set_stack("main", i, ItemStack("sl_weapons:pistol"))
