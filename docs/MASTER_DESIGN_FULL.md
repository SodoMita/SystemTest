...
| Essence generated by defender activity | MM crafts essence itself | **divergence — RESOLVED 2026-09-02**, see 13.3 |
...
### 13.3 The essence engine — RESOLVED (owner ruling, 2026-09-02)

The owner's loop is accepted, with the **source of essence corrected** from the
draft (machine operations) to what the ruling specifies: the MM's essence is
earned by **destruction of nodes the crew has placed, scaled by the node's
price**. This mostly implements reward by *crew success*: the more the crew
invests in the arena, the more the MM is paid when that investment is destroyed.
Cheap scaffolding pays nothing; a beacon bastion, a redoubt, or a fortified
machine pays out. Some crafts also pay: **the objective core** — the crew's
biggest statement — pays the pool directly when completed.

> **Rules**
> 1. Destroying a crew-placed node credits the MM pool `essence = price(node)`,
>    where `price` is the node's build cost from the crafting/salvage economy
>    (§6.10). Only crew-placed nodes count — map, worldgen and engine nodes do
>    not pay; the map system's node provenance is the bookkeeper.
> 2. Certain crafts credit the pool on completion — the **objective core**
>    (+3) is the named example; the same hook scales to the Objective Forge.
> 3. **Essence is not a score.** Points are a separate ladder and point gain
>    comes **primarily from killing crew**. Were essence itself scored, it
>    would promote farming (build → destroy → rebuild) instead of the contest
>    the match is about. The scoreboard stays kills + objectives; essence stays
>    fuel.
> 4. If there is no MM in the match, the pool accrues into **ambient hazard**:
>    at thresholds, one automated security unit spawns from the Node itself
>    (unchanged from the minimum version).
...
2. ~~**Essence from crew activity (§13.3) — yes or no?**~~ **RESOLVED 2026-09-02 — yes**,
   with the mechanism ruled in §13.3: destruction of crew-placed nodes pays
   `price(node)`, select crafts (objective core, +3) pay directly, essence is
   fuel and **not** score, and points come primarily from killing crew.
