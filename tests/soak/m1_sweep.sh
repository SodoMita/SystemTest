#!/usr/bin/env bash
# M1 exit-check sweep (see AGENT_PARALLEL_PLAN.md milestone M1):
# 40 matches across four regimes. Exit criteria evaluated by
# tests/soak/m1_summary.py over the emitted reports:
#   * 0 bug events, 0 engine errors in every run
#   * both elimination wins AND timer draws occur
#   * every event counter > 0 across the sweep
set -u
cd "$(dirname "$0")/../.."
OUT=${1:-/tmp/m1_sweep}
mkdir -p "$OUT"

run() { # label, extra args...
  local label=$1; shift
  echo "[sweep] === $label ==="
  python3 tests/soak/run_soak.py "$@" --timeout 1800 \
    --report "$OUT/$label.json" > "$OUT/$label.log" 2>&1
  echo "[sweep] $label exit=$? -> $OUT/$label.log"
}

# A: elimination-focused (turbo: adjacent bases, matches resolve in seconds)
run a_elim  --turbo --matches 10 --match-duration 60 --seed 101
# B: timer-forced (single life: clock must beat the wipeout -> draws)
run b_timer --turbo --matches 10 --match-duration 4  --seed 202
# C: mixed standard regime (turbo clocks, moderate lives)
run c_mixed --turbo --matches 10 --match-duration 25 --seed 303
# D: wide roster (6 bots, 3v3)
run d_wide  --turbo --matches 10 --bots 6 --match-duration 45 --seed 404

python3 tests/soak/m1_summary.py "$OUT"
echo "SWEEP_DONE rc=$?"
