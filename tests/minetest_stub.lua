...
-- Engine parity: vector.safe_dir / vector.finite (builtin
-- common/vector.lua, Luanti 5.7+/5.9+). The mods use these as the
-- NaN armor layer (W1g): any vector about to reach a movement write
-- passes through safe_dir, and knockback drops non-finite shoves.
-- Without them in the stub the W1g point-blank-mortar regression
-- suite cannot run at all.
function vector.finite(v)
	if type(v) ~= "table" then return false end
	return type(v.x) == "number" and type(v.y) == "number" and type(v.z) == "number"
		and v.x == v.x and v.y == v.y and v.z == v.z -- not NaN
